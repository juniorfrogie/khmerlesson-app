...
## Bug Fix: Stale "Plan Active" on Me Tab After Token Expiry (2026-09-01)

**User report**: on app load, courses show locked, but the Me tab shows a subscription plan as active — reported independently for both Plan 1 and Plan 3, ruling out a per-plan course-mapping data issue (the first, wrong diagnosis this session initially reached — see below). User's own follow-up nailed the real mechanism: "after switching back and forth, app redirect to login... token expire but app still show stale data before enforce login."

**Root cause, confirmed by code inspection**: `subscriptionStore.setError()` (`src/features/subscriptions/store/subscriptionStore.ts`) deliberately never clears `mySubscription` on a failed sync — by design, so a transient network blip doesn't make previously-unlocked content flash as locked. But `syncSubscription()` (`src/features/subscriptions/service.ts`) called `setError()` for *every* failure, including a definitively dead session (access token expired **and** the refresh retry also failed, or a blacklisted/revoked token) — so the Me tab kept showing the last-persisted "Plan X Active" from AsyncStorage indefinitely. Meanwhile `GET /api/v1/main-lessons` (courses) has no such cache — it's computed fresh server-side per request, so it correctly (if confusingly, next to the stale Me tab) showed no access. The eventual redirect to login only happened once the user navigated to a screen with its own auth check (`me.tsx`'s `isAuthenticated` effect, or a course/lesson screen's `forbiddenReason` handling) — Home/Explore/Quiz tabs had no such check at all, so a dead session could sit silently unresolved until then.

**(Wrong first diagnosis, corrected by the user)**: initially attributed this to Plan 3 having no courses assigned via `subscription_plan_courses` (a real, separate thing that's still worth checking in the admin dashboard if it recurs for one specific plan only — but not this bug, since it reproduced on Plan 1 too).

**Fixes**:
- `src/features/subscriptions/service.ts` — `syncSubscription()` now distinguishes a definitive auth failure (`code === 'TOKEN_EXPIRED'` or `'TOKEN_REVOKED'`) from any other error. On a definitive failure it calls `authStore.signOut()` (which also clears subscription state) instead of `setError()`; every other error still preserves stale data exactly as before — the original transient-failure protection is untouched, just no longer applied to a case it was never meant to cover.
- `app/_layout.tsx` — new global subscription to `useAuthStore`, redirecting to `/auth/login` immediately on any transition from authenticated → fully signed-out, regardless of which tab is active. Closes the gap where Home/Explore/Quiz had no auth guard at all, so a sign-out (from the fix above, or anywhere else) previously only took effect once the user happened to land on a screen that separately checked auth state.
- **khmerlesson-dashboard** `server/auth/middleware/authenticate.ts` — smaller, related correctness fix found while tracing this: a *blacklisted* token on a semi-public route (e.g. `main-lessons`) was hard-401'ing (`code: TOKEN_REVOKED`) instead of falling through anonymously like an expired/invalid token already does — inconsistent, and `TOKEN_REVOKED` isn't in the mobile client's retry-after-refresh code list, so this specific case couldn't self-heal at all. Now mirrors the existing JWT-verify-failure branch: semi-public routes get `req.tokenInvalid = true` + fall through, other routes keep the hard 401.
