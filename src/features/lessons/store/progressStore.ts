  ...
  try {
    ...
    if (legacyRaw) {
      ...
      logger.info(newTraceId(), 'legacy_progress_migrated', {
        store: 'lesson',
        courseCount: Object.keys(legacy.completedLessons ?? {}).length,
      });
    }

    await AsyncStorage.setItem(LEGACY_MIGRATION_FLAG_KEY, 'true');
  } catch (err) {
    logger.warn(newTraceId(), 'legacy_progress_migration_failed', { store: 'lesson', message: (err as Error).message });
